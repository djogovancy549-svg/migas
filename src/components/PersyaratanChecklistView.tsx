...
export const PersyaratanChecklistView: React.FC<PersyaratanChecklistViewProps> = ({
  ...
  const currentRekomendasi = activePangkalanId ? rekomendasiMap[activePangkalanId] : undefined;
          ...
          {!currentPangkalan ? (
            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {/* Rekomendasi Direct Action Button */}
                  <button
                    onClick={() => onOpenRekomendasiModal(currentPangkalan)}
                    className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold rounded-xl text-xs transition cursor-pointer shadow-md shadow-amber-500/10 min-h-[38px]"
                  >
                    <Award className="w-4 h-4" />
                    <span>Aksi Rekomendasi</span>
                  </button>

                  {/* Upload / Folder Persyaratan Trigger */}
                  <button
                    onClick={() => currentPangkalan && onOpenUploadModal(currentPangkalan)}
                    className={`inline-flex items-center gap-1.5 px-3 py-2 font-bold rounded-xl text-xs transition cursor-pointer border shadow-md min-h-[38px] ${
                  ...
                  >
                    {currentUploadedDocs.length > 0 ? (
                      <>
                        ...
                        <span>Berkas ({currentUploadedDocs.length})</span>
                      </>
                    ) : (
                      <>
              ...
              <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <span>{isAllComplete ? 'BERKAS LENGKAP ✓' : 'BELUM LENGKAP'}</span>
                  </span>

                  {/* Prominent Action Button when requirements are completed */}
                  <button
                    onClick={() => onOpenRekomendasiModal(currentPangkalan)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black rounded-lg text-xs shadow-md transition cursor-pointer"
                  >
                    <span>{isAllComplete ? 'Proses Rekomendasi TTD' : 'Lihat Status Rekomendasi'}</span>
